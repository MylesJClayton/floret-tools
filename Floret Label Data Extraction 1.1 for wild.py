...
for FloretFilename in FloretFileNamesList:
    ...
    EmbryoMask = sitk.BinaryThreshold( FloretLabel, 6, 6, 1, 0 )
    
    #Merge masks
    TotalGrainMask = GrainMask + EmbryoMask + BranMask
    ...
    while Iterationcounter < FillIterations:
        CavityDilation = DilateImage.Execute(CavityDilation)
        CavityDilation = CavityDilation - ShellMask + 1
    ...
    while ErodeCounter < ErodeIterations:
        CavityLabel = ErodeImage.Execute(CavityLabel)
        ErodeCounter = ErodeCounter + 1
        
    print("dilating to refill cavity")
    while dilationcounter3 < ErodeIterations + 1:
        CavityLabel = DilateImage.Execute(CavityLabel)
        dilationcounter3 = dilationcounter3 + 1
    
    CavityLabel = CavityLabel - ShellMask + 1
    CavityLabel = sitk.BinaryThreshold( CavityLabel, 2, 2, 1, 0 )
    print("done")
